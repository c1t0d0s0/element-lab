...
export const TRANSLATIONS = {
  ja: {
    ...
    tooltips: {
      spawn: '元素・化合物を配置',
      balance: '精密上皿天秤を配置して元素や化合物の重さを比較',
    ...
    balance: {
      title: '精密上皿天秤',
      subtitle: 'ローベルバル天秤 (Roberval Mechanism)',
      leftPan: '左の皿',
      rightPan: '右の皿',
      balanced: '⚖️ つり合っています (Balanced)',
      leftHeavy: '◀ 左が重い',
      rightHeavy: '右が重い ▶',
      massDiff: '質量差',
      massUnit: 'g/mol',
      ...
      clearRight: '右皿を空にする',
      clearAll: '両皿を空にする',
      emptyPan: '（皿の上に物質がありません）',
      ratio: '質量比',
      mextKnowledgeTitle: '📘 上皿天秤の使い方（理科・化学）',
      mextKnowledgeDesc: '上皿天秤は左右の皿にかかる重力（質量）を比較する器具です。水平な場所に置き、あらかじめ指針が目盛りの中央で等しく振れるようゼロ点を合わせます。物質を量るときは、ピンセットを使って分銅をのせ、つり合ったら分銅の合計質量を読み取ります。'
    },
    chamber: {
    ...
    inspector: {
      ...
      capOpen: '開放 (無栓) 🔓',
      btnAddCap: '🧴 蓋をする (密閉)',
      btnRemoveCap: '💨 蓋を外す (開放)',
      capDescClosed: 'フラスコが密閉されています。発生した気体や蒸気が逃げずに内部に保持されます。',
      capDescOpen: 'フラスコが開いています。軽い気体は上部から実験室へ拡散します。',
      tareMass: '風袋重量 (容器自重)',
      totalContainerMass: '総質量 (容器+内容物)',
      containerState: '配置状態',
      stateOnLeftPan: '⚖️ 天秤 (左皿に載置)',
      stateOnRightPan: '⚖️ 天秤 (右皿に載置)',
      stateOnFloor: '🧪 実験台 (床面)',
      stateFalling: '🪂 落下中',
      innerContent: '内部の物質',
    ...
    toasts: {
      ...
      balancePlaced: '⚖️ 精密上皿天秤を設置しました！',
      balanceCleared: '⚖️ 天秤の皿を空にしました',
      capAdded: (name: string) => `🧴 ${name} に蓋を付けました（気体を密閉）`,
  ...
  en: {
    ...
    tooltips: {
      ...
      balance: 'Place pan balance to compare mass and molar weights of elements & compounds',
    ...
    balance: {
      title: 'Precision Pan Balance',
      subtitle: 'Roberval Mechanical Balance',
      leftPan: 'Left Pan',
      rightPan: 'Right Pan',
      balanced: '⚖️ Balanced',
      leftHeavy: '◀ Left is Heavier',
      rightHeavy: 'Right is Heavier ▶',
      massDiff: 'Mass Difference',
      massUnit: 'g/mol',
      lock: '🔒 Lock Beam',
      unlock: '🔓 Release Beam',
      clearLeft: 'Clear Left Pan',
      clearRight: 'Clear Right Pan',
      clearAll: 'Clear Both Pans',
      emptyPan: '(No particles on pan)',
      ratio: 'Mass Ratio',
      mextKnowledgeTitle: '📘 How to Use a Pan Balance (Science Curriculum)',
      mextKnowledgeDesc: 'A pan balance compares masses placed on two pans using gravity and lever principles. It must be placed on a level table, and zeroed so the needle oscillates evenly around the central mark. Always use tweezers when handling standard weights.'
    },
    chamber: {
    ...
    inspector: {
      ...
      capStatus: 'Sealing Status',
      capClosed: 'Sealed (Stopper On) 🔒',
      capOpen: 'Open (No Stopper) 🔓',
      btnAddCap: '🧴 Put Cap On (Seal)',
      btnRemoveCap: '💨 Remove Cap (Open)',
      capDescClosed: 'The flask is sealed with a stopper. Generated gases and vapors are securely contained inside.',
      capDescOpen: 'The flask is open. Light gases will rise and diffuse into the laboratory chamber.',
      tareMass: 'Tare Mass (Empty)',
      totalContainerMass: 'Total Mass (Tare + Content)',
      containerState: 'Placement',
      stateOnLeftPan: '⚖️ On Balance (Left Pan)',
      stateOnRightPan: '⚖️ On Balance (Right Pan)',
    ...
    toasts: {
      ...
      balancePlaced: '⚖️ Precision pan balance placed!',
      balanceCleared: '⚖️ Cleared balance pans',
      capAdded: (name: string) => `🧴 Sealed ${name} with stopper/cap!`,
